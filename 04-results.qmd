...
```{r parameter400, echo = FALSE, fig.align="center", results='asis', out.width = "75%", fig.cap = "HBW Distributions for Input Parameters with 400 Draws"}
tar_load(hbw_mc_coeff_lists_400)
list400 <- hbw_mc_coeff_lists_400

Z <- bind_rows(list400) %>% 
  mutate(method = c("base", rep("MC", 400), rep("LHS", 400))) %>%
  mutate(vot = (ivtt/ccost) * (60/100)) %>%
  pivot_longer(cols = -c("method"))
...
```{r hbwstats, warning = FALSE, echo = FALSE, message = FALSE, fig.align="center", out.width = "75%", results='asis', fig.cap = "HBW Mean Logsum Standard Variation with 100 and 400 Draws"}
tar_load(hbw_stats_100)
tar_load(hbw_stats_400)
hbw_stats <- bind_rows(hbw_stats_100, hbw_stats_400, .id = "ndraws") %>%
...
```{r hbostats, warning = FALSE, echo = FALSE, message = FALSE, fig.align="center", out.width = "75%", results='asis', fig.cap = "HBO Mean Logsum Standard Variation with 100 and 400 Draws"}
tar_load(hbo_stats_100)
tar_load(hbo_stats_400)
hbo_stats <- bind_rows(hbo_stats_100, hbo_stats_400, .id = "ndraws") %>%
  mutate(ndraws = case_when(ndraws == "1" ~ "100 Draws",
                          ndraws == "2" ~ "400 Draws"))
...
```{r nhbstats, warning = FALSE, echo = FALSE, message = FALSE, fig.align="center", out.width = "75%", results='asis', fig.cap = "NHB Mean Logsum Standard Variation with 100 and 400 Draws"}
tar_load(nhb_stats_100)
tar_load(nhb_stats_400)
nhb_stats <- bind_rows(nhb_stats_100, nhb_stats_400, .id = "ndraws") %>%
  mutate(ndraws = case_when(ndraws == "1" ~ "100 Draws",
                          ndraws == "2" ~ "400 Draws"))
